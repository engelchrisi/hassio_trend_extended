...
from homeassistant.components.trend_extended.const import DOMAIN
...
async def _setup_legacy_component(hass: HomeAssistant, params: dict[str, Any]) -> None:
    ...
    assert await async_setup_component(
        ...
        {
            "binary_sensor": {
                "platform": "trend_extended",
                "sensors": {
                    "test_trend_extended_sensor": params,
                },
            }
        },
    )
...
async def test_basic_trend_extended_setup_from_yaml(
    hass: HomeAssistant,
    states: list[str],
    inverted: bool,
    expected_state: str,
) -> None:
    """Test trend_extended with a basic setup."""
    ...
    assert (sensor_state := hass.states.get("binary_sensor.test_trend_extended_sensor"))
...
async def test_basic_trend_extended(
    hass: HomeAssistant,
...
async def test_using_trendline(
    ...
    for idx, states in enumerate(state_series):
        ...
        assert (
            sensor_state := hass.states.get("binary_sensor.test_trend_extended_sensor")
        )
...
async def test_max_samples(
    ...
    assert (state := hass.states.get("binary_sensor.test_trend_extended_sensor"))
...
async def test_no_sensors_does_not_create(hass: HomeAssistant) -> None:
    ...
    with assert_setup_component(0):
        assert await setup.async_setup_component(
            hass, "binary_sensor", {"binary_sensor": {"platform": "trend_extended"}}
        )
    assert hass.states.async_all("binary_sensor") == []
...
async def test_restore_state(
    ...
    mock_restore_cache(
        hass, (State("binary_sensor.test_trend_extended_sensor", saved_state),)
    )
    ...
    assert (
        hass.states.get("binary_sensor.test_trend_extended_sensor").state
        == restored_state
    )
    ...
    assert hass.states.get("binary_sensor.test_trend_extended_sensor").state == "on"
...
async def test_invalid_min_sample(
    ...
    assert (
        "Invalid config for 'binary_sensor' from integration 'trend_extended': min_samples must "
...
async def test_device_id(
    ...
    trend_entity = entity_registry.async_get("binary_sensor.trend_extended")
